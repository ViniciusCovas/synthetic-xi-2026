...
DEFAULT_STATUS = Path("data/model_readiness/official_experiment_v1_preflight_status.json")
DEFAULT_AUTHORIZATION = Path("data/model_readiness/official_release_authorization_v1.json")
...
def _canonical_structural_checks(config: dict[str, Any]) -> tuple[dict[str, bool], dict[str, Any]]:
    ...
    checks = {
        ...
        "no_provisional_profiles": all(
            row.get("provisional") is False for row in synthetic_rows + real_rows
        ),
        "no_exploratory_labels": all(
            "explor" not in name.lower()
            for name in (synthetic.team.name, real.team.name)
        ),
        "selection_sufficiency": bool(selection.get("selection_sufficiency_gate_passed")),
        "historical_eleven_role_gate": bool(historical_roles.get("eleven_role_gate_passed")),
        "rankings_allowed": bool(rankings.get("official_rankings_allowed")),
        "external_holdout_passed": bool(holdout.get("external_pre_tournament_validation_passed")),
        "final_team_comparison_allowed": bool(scientific.get("final_team_comparison_allowed")),
        "protocol_amendment_present": Path(config["protocol_amendment"]).exists(),
        "post_result_tuning_performed_false": config.get("post_result_tuning_performed") is False,
        "engine_version_frozen": config.get("engine_version") == OFFICIAL_ENGINE_VERSION,
    }
    evidence = {
        "selection_status": selection.get("status"),
        "historical_role_status": historical_roles.get("status"),
        "historical_rankings_allowed_preserved": historical_roles.get("rankings_allowed"),
...
def _release_checks(
    ...
    checks = {
        "validation_engine_version": simulation.get("version") == OFFICIAL_ENGINE_VERSION,
        "validation_simulation_count": int(simulation.get("simulations", 0))
        >= int(config["analysis"]["validation_simulations"]),
        "canonical_rosters_match_runtime": bool(
            simulation.get("canonical_runtime", {}).get("synthetic_registered_ids")
            and simulation.get("canonical_runtime", {}).get("real_registered_ids")
        ),
        "no_provisional_profiles": bool(
            simulation.get("canonical_runtime", {}).get("no_provisional_profiles")
        ),
        "no_exploratory_labels": bool(
            simulation.get("canonical_runtime", {}).get("no_exploratory_labels")
        ),
        "event_sanity_passed": bool(simulation.get("event_sanity_gate", {}).get("passed")),
        ...
        "seed_stability_passed": bool(simulation.get("seed_stability", {}).get("passed")),
        ...
        "manifest_engine_version": manifest.get("engine_version") == OFFICIAL_ENGINE_VERSION,
        "manifest_source_hashes_match": manifest.get("source_hashes") == source_hashes,
        "post_result_tuning_performed_false": config.get("post_result_tuning_performed") is False,
    }
    evidence = {
        "validation_summary": str(validation / "simulation_summary.json"),
        "hypotheses_summary": str(validation / "official_hypotheses_summary.json"),
        "sensitivity_summary": str(robustness / "official_sensitivity_summary.json"),
        "nested_summary": str(robustness / "official_nested_uncertainty_summary.json"),
...
def main() -> None:
    ...
    status = {
        ...
        "post_result_tuning_performed": config["post_result_tuning_performed"],
    }
