...
def main() -> None:
    ...
    if not players.empty:
        ...
        players["minutes_num"] = pd.to_numeric(players["minutes"], errors="coerce").fillna(0.0)
        action_cols = [
            "passes_total",
            "shots_total",
            "tackles_total",
    ...
    if not lineups.empty:
        lineup_players = set(lineups["player_id"].astype(int))
    else:
        lineup_players = set()
    ...
    if not active.empty:
        player_summary = (
            ...
            .agg(
                ...
                current_window_fixtures=("in_current_window", lambda s: int(pd.Series(s).astype(str).str.lower().eq("true").sum())),
                pre_world_cup_fixtures=("in_pre_world_cup_window", lambda s: int(pd.Series(s).astype(str).str.lower().eq("true").sum())),
                provider_position_rate=("provider_position", lambda s: float(s.notna().mean())),
                passes_rate=("passes_total", lambda s: float(s.notna().mean())),
            )
        )
        player_summary["has_lineup_evidence"] = player_summary["player_id"].astype(int).isin(lineup_players)
        ...
        player_summary.to_csv(AUDIT_DIR / "adaptive_player_coverage.csv", index=False)
    else:
        player_summary = pd.DataFrame()

    completed = int(progress["fixture_id"].nunique()) if not progress.empty else 0
    endpoint_empty = int((progress["status"] == "player_endpoint_empty").sum()) if not progress.empty else 0
    no_target = int((progress["status"] == "no_target_player_returned").sum()) if not progress.empty else 0
    completed_with_targets = int((progress["status"] == "completed").sum()) if not progress.empty else 0

    players_seen = int(player_summary["player_id"].nunique()) if not player_summary.empty else 0
    benchmarks_seen = 0
    if not player_summary.empty:
        benchmarks_seen = int(
            player_summary["benchmark_precheck"].astype(str).str.lower().eq("true").sum()
        )
    ...
    status = {
        ...
        "active_minutes_nonnull_rate": float(active["minutes_num"].notna().mean()) if not active.empty else 0.0,
        "active_position_nonnull_rate": float(active["provider_position"].notna().mean()) if not active.empty else 0.0,
        "active_passes_nonnull_rate": float(active["passes_total"].notna().mean()) if not active.empty else 0.0,
        "methodological_gate": {
